...
class TestHomePage(TestCase):
    ...
    def setUpTestData(cls):
        test_news = [
                News(
                    title=f'Test News {index}',
                    text='Test content',
                    date=datetime.today() - timedelta(days=index),
                )
                for index in range(settings.NEWS_COUNT_ON_HOME_PAGE + 1)
            ]
        News.objects.bulk_create(test_news)

    def test_news_count(self):
        response = self.client.get(self.HOME_URL)
        objects_list = response.context['news_list']
        self.assertEqual(
            len(objects_list),
    ...
    def test_news_order(self):
        response = self.client.get(self.HOME_URL)
        objects_list = response.context['news_list']
        dates_list = [object.date for object in objects_list]
        dates_sorted = sorted(dates_list, reverse=True)
        self.assertEqual(
            dates_list,
            dates_sorted
        )
